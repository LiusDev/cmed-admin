...
const Qutes2Modal = forwardRef(
    (
        ...
        const oldData = useMemo(() => {
            if (index != null) return props.form.values.quotes2[index]
            return undefined
        }, [index])
        ...
        return (
            ...
            >
                <Flex direction={"column"} gap={"15px"}>
                    <TextInput
                        title="Tiêu đề"
                        {...props.form.getInputProps(`quotes2.${index}.title`)}
                    />
                    <CustomEditor
                        value={
                    ...
                    <ImageInput
                        title="Ảnh"
                        {...props.form.getInputProps(`quotes2.${index}.image`)}
                    />
                    <Group>
...
const Quotes2Row = ({
    ...
    value: ElementOf<AboutPage["quotes2"]>
    onEdit: () => void
    onDelete: () => void
    onChange: (value?: ElementOf<AboutPage["quotes2"]>) => void
}) => {
    return (
        <Table.Tr>
            ...
            <Table.Td>
                <Group justify="center">
                    <Button variant="filled" color="green" onClick={onEdit}>
                        Sửa
                    </Button>
...
const Edit = () => {
    ...
    useEffect(() => {
        ;(async () => {
            const {
                data: { content: data },
            } = await instance.get<{ content: AboutPage }>(
                `/setting/item/about/${lang}`
            )
            data.content2 = (data.content2 as string[])
                .map((i) => {
                    let text = i
                    if (!i.startsWith("<p>")) text = "<p>" + text

                    if (!i.endsWith("</p>")) text = text + "</p>"
                    return text
                })
                .join("")
            form.setValues(data)
    ...
    const submit = useCallback(() => {
        ...
        const newContent2 = Array.from(div.children)
            .filter((i) => i.tagName === "P")
            .map((i) => i.innerHTML)
        instance
            .put(`/setting/about/${lang}`, { content2: newContent2, ...rest })
            .then(() => {
                setLoading(false)
    ...
    return (
        <MainLayout>
            <Breadcrumb
                pageName="Thiết lập trang về chúng tôi"
                link="/settings"
            />
            <Box className="max-w-230 m-auto">
                ...
                <div className="flex flex-col gap-5.5 p-6.5">
                    ...
                    <Skeleton visible={loading}>
                        ...
                        />
                        <TextInput
                            title="Nội dung nút 1"
                            {...form.getInputProps("featuredButtonTitle")}
                        />
                        <TextInput
                            title="Nội dung nút 2"
                            {...form.getInputProps("featuredButtonTitle2")}
                        />
                        <Divider my="md" />
                        <TextInput
                            title="Tiêu đề tab 1"
                            {...form.getInputProps("tabTitle1")}
                        />
                        <TextInput
                            title="Tiêu đề tab 2"
                            {...form.getInputProps("tabTitle2")}
                        />
                        <TextInput
                            title="Tiêu đề tab 3"
                            {...form.getInputProps("tabTitle3")}
                        />
                        <Divider my="md" />
                        <TextInput
                            title="Tiêu đề 2"
                        ...
                        <TextInput
                            ...
                            {...form.getInputProps("quotes1.content")}
                        />
                        <TextInput
                            title="Tác giả quote"
                            {...form.getInputProps("quotes1.author")}
                        />
                        <ImageInput
                            title="Ảnh nền quote"
                            {...form.getInputProps("quotes1.background")}
                        />
                        <Divider my="md" />
                        <Quotes2Table
                            form={form}
                            {...form.getInputProps("quotes2")}
                        />
                    </Skeleton>
                    <div>
                        <CustomButton
